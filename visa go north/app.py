...
def evaluate_client(data):
    ...
    # --- أ. الملاءة المالية (30 نقطة) - المنطق الجديد المضاف ---
    has_bank = data.get("has_bank_account")
    
    if has_bank == "no":
        score += 0
        improvement.append("عدم وجود حساب بنكي يعتبر عائقاً كبيراً؛ ننصح بفتح حساب وإيداع مبالغ دورية لمدة 6 أشهر قبل التقديم.")
    else:
        # تحويل الرصيد مع معالجة حالة الفراغ لتجنب الأخطاء
        raw_balance = data.get("bank_balance", "0")
        balance = int(raw_balance) if raw_balance.strip().isdigit() else 0
    ...
    salary = int(data.get("salary", 0) if data.get("salary") else 0)
    if salary >= 15000:
        score += 5
    ...
    trip_days = int(data.get("trip_duration_days", 7) if data.get("trip_duration_days") else 7)
    if trip_days > 15:
        score -= 5
    ...
    return {
        ...
        "score": score,
        "suggestion": " / ".join(final_strategy["countries"]) if final_strategy.get("countries") else "لا ننصح بالتقديم حالياً",
        "main_reason": final_strategy.get("status", "غير محدد"),
        "reasons": reasons,
        "improvement": improvement,
        "chosen_message": chosen_message,
        "evidence": final_strategy.get("evidence", ""),
        "why_no": final_strategy.get("why_no", "")
    }
